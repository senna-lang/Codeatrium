...
| エージェントから呼びやすいか | サーバー起動 + curl 経由 | `loci search "..." --json` で完結 |
...
```bash
loci search "connection pool 枯渇"
...
```bash
loci context --symbol "AuthMiddleware.validate"
```

### 原文取得

```bash
loci show "~/.claude/projects/.../abc.jsonl:ply=42"        # 人間向け
loci show "~/.claude/projects/.../abc.jsonl:ply=42" --json  # LLM向け
```

### インデックス管理

```bash
loci init                         # .codeatrium/ を初期化（初回のみ）
loci index                        # 未インデックスの .jsonl を処理
loci index --path ~/.claude/projects/...
loci distill                      # 未蒸留の exchange を Haiku で処理
loci status                       # インデックス状況を表示
```

### 全件 in-context ロード（セッション開始時）

```bash
loci dump --distilled                     # 全 distilled objects を出力（デフォルト: 最新1000件）
loci dump --distilled --limit 500         # 件数指定
loci dump --distilled --json              # JSON 配列で出力（LLM 向け）
...
> RAG（`loci search`）は「何を探すか分かっている」場合に使い、
> `loci dump` は「セッション開始時に全記憶を俯瞰したい」場合に使う。両者は補完関係。

### LLM 向け（JSON 出力）

```bash
loci search "connection pool" --json --limit 5
...
### ストレージ（`<project-root>/.codeatrium/`）

`loci init` でプロジェクトルートに初期化する（`.git` と同じモデル）。
...
`loci index` 実行時に `.jsonl` ファイル一覧と DB を突き合わせて未処理分のみ処理する。
...
exchanges (
  ...
  user_content TEXT,
  agent_content TEXT,
  distilled_at TIMESTAMP     -- NULL = 未蒸留・loci distill の処理対象判定に使用
...
Claude Code ラリー終了（毎ターン）
  └─ Stop フック（async: true）
       └─ loci index          （ノンブロッキング / embedding のみ）

セッション境界（/clear・/resume・CC終了）
  └─ SessionEnd フック
       └─ nohup loci distill &（detach・CC終了後も継続）
...
~/.claude/projects/<project-hash>/*.jsonl
  │  ※ project-hash は git root のパスから自動解決（loci init 時に記録）
  │
...
蒸留キュー
  ...
  │
  │   ※ 蒸留は loci distill の1モードのみ。常に room_assignments を生成する。
  │     Stop フックで nohup loci distill & としてバックグラウンド実行するため
...
- [ ] `loci search` で HNSW 検索
- [ ] `loci index` で手動インデックス
...
- [ ] symbols テーブルへの保存
- [ ] `loci context --symbol` でコード→会話の逆引き

### Phase 4: 自動化 + 全件ロード

- [ ] Claude Code **Stop フック**（`async: true`）で `loci index` を自動実行（毎ターン・ノンブロッキング）
- [ ] Claude Code **SessionEnd フック**（`nohup ... &`）で `loci distill` をセッション境界時のみ起動
      （`/clear`・`/resume`・CC終了をトリガーとし、毎ターンの蒸留コストを回避）
- [ ] `loci hook install` で両フックを `~/.claude/settings.json` に一括登録
- [ ] CLAUDE.md テンプレート（LLM が自動で呼び出す設定）
- [ ] `loci status` でインデックス状況の可視化
- [ ] `loci dump --distilled [--limit N] [--json]` — 全件 in-context ロード用コマンド
...
- [x] **ツール名**: codeatrium、コマンド名 `loci` で確定
...
- [x] **マルチエージェント対応**: Phase 1 は Claude Code（`~/.claude/projects/**/*.jsonl`）固定。
      ...
      拡張時の設計方針:
        ...
        - `loci init --agent <claude|codex|...>` で対象を指定
...
プロジェクトの `CLAUDE.md` に以下を追加することでエージェントが自律的に codeatrium を使えるようになる。
...
\`\`\`bash
loci context --symbol "AuthMiddleware.validate" --json
\`\`\`
````
...
**症状**: Stop フックに `loci index && loci distill` を同期実行で登録したため、Claude が応答を返すたびに数秒待ちが発生し UX が壊滅した。

**原因**: `loci index` が同期実行でブロックしていた。

**解決**: Stop フックを `async: true`（CC ネイティブ非同期）に変更し、`loci index` をノンブロッキング化。`loci distill` は Stop フックから外してセッション境界のみで実行するよう分離した。

---

### 問題② SessionEnd フックの無限ループ

**症状**: `loci distill` を登録した SessionEnd フックが多重起動し、複数の `claude --print` プロセスが積み重なった。最長で数時間前から走り続けるプロセスが残っていた。

**原因**: `claude --print` 自体が CC セッションとして扱われ、そのセッション終了時に SessionEnd フックが再発火するループが発生していた。

```
SessionEnd → loci distill → claude --print → SessionEnd → loci distill → ...
```
...
**症状**: `loci distill --limit 1` を実行しただけで CC Pro のレートリミットに当たった。
...
**症状**: 複数の CC インスタンス（Zed・ターミナル・他プロジェクト）が同時に SessionEnd/SessionStart を発火し、`loci distill` プロセスが並列に複数起動した。

**解決**: ファイルベースのプロセスロックを実装。起動時に `.codeatrium/distill.lock` に PID を書き込み、起動済み PID が生きていれば即 exit。強制終了後の残骸ロックも PID 死活確認で自動クリア。
...
**症状**: `loci distill` 実行時に `OperationalError: UNIQUE constraint failed on vec_palace primary key` が発生。
...
## 12. `loci search` パフォーマンスチューニング

### 問題: 毎回 7 秒かかるコールドスタート

**症状**: `loci search` を実行するたびに 7〜8 秒かかる。エージェントが自律的に使うには許容できない遅延。

**原因**: `loci search` は毎回新しい Python プロセスを起動し、`multilingual-e5-small`（500MB）のモデル重みをゼロからロードしていた。

```
$ time loci search "RRF 採用理由"
...
モデルを常駐させる軽量サーバーを実装し、`loci search` はソケット経由でクエリを投げるだけにした。
...
loci search "..." 
  │
  ├─ .codeatrium/embedder.sock が存在する？
...
embedder_server（常駐プロセス）
  - Unix ソケット .codeatrium/embedder.sock でリッスン
  - モデルをメモリに保持
  - リクエストごとにスレッドで処理
...
- `SessionStart` フックで `nohup loci server start > /dev/null 2>&1 &` を実行 → CC 起動時に自動ウォームアップ
...
- `loci context --symbol` の結果が要約テキストで汚染される
- トークン無駄遣い（要約は数千文字に及ぶ）
...
蒸留時に `files_touched` からシンボルを tree-sitter で抽出し、`symbols` テーブルに保存する。`loci context --symbol <name>` で「このシンボルに関連する過去会話」を逆引きできる。
...
CLAUDE.md に以下を記述しておくことで、CC がファイルを編集しようとしたとき `loci context --symbol` を自動実行できる：

```bash
loci context --symbol "変更対象の関数名" --json
```
...
- **`loci context --file <path>` の充実**: ファイル単位での逆引きをシンボル一覧とともに返す
...
- **初回モデルダウンロード**: `loci init` 時に約500MBのモデルを自動ダウンロードする必要がある。ユーザーへの告知・プログレス表示の設計を実装時に検討する。
- **チーム共有は現状スコープ外**: `source_path` がローカルパス依存・SQLite がバイナリで git マージ不能なため、現状はパーソナルツールとして割り切る。将来的にやるなら palace objects を JSONL エクスポートして `loci import` で取り込む設計が候補。
