"""
logo status / logo hook install のテスト
...
def _setup_db(tmp_path: Path) -> Path:
    """テスト用 DB を初期化して logosyncs ディレクトリを作成する"""
...
def test_status_not_initialized(tmp_path, monkeypatch):
    ...
    assert "logo init" in result.output
...
def test_hook_install_creates_settings(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("logo.cli.Path.home", lambda: tmp_path)
...
def test_hook_install_idempotent(tmp_path, monkeypatch):
    ...
    logo_hooks = [h for h in all_hooks if "logo index" in h.get("command", "")]
    assert len(logo_hooks) == 1
